...
-- Таблица для связи соревнований с городами
CREATE TABLE IF NOT EXISTS competition_locations (
    ...
    PRIMARY KEY (competition_id, location)
);

-- Таблица для связи соревнований с направлениями
...
-- Таблица для связи соревнований с типами участия
...
CREATE TABLE IF NOT EXISTS applications (
    ...
    status VARCHAR NOT NULL, -- Статус заявки (отправлена, принята, отклонена)
    CONSTRAINT fk_application_competition FOREIGN KEY (competition_id) REFERENCES competitions (id) ON DELETE CASCADE,
    CONSTRAINT fk_application_user FOREIGN KEY (user_id) REFERENCES users (id) ON DELETE CASCADE
);

-- Таблица для связи заявок с направлениями
